...
class SubirPDF(commands.Cog):
    ...
    async def subir_pdf(self, interaction: discord.Interaction, zip_file: discord.Attachment, serie: str, chapter: int):
        ...
        try:
            ...
            await interaction.followup.send(f"Archivo ZIP {zip_file.filename} guardado.")
        except Exception as e:
            logging.error(f"Error al guardar el archivo ZIP: {str(e)}")
            await interaction.followup.send(f"Error al guardar el archivo ZIP: {str(e)}")
            return
        ...
        try:
            ...
            await interaction.followup.send(f"PDF subido a Google Drive para la serie {serie}, capítulo {chapter}.")
        except Exception as e:
            logging.error(f"Error al subir el PDF a Google Drive: {str(e)}")
            await interaction.followup.send("Error al subir el PDF a Google Drive.")
            return
